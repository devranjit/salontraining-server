...
export async function getAllTrainers(req: Request, res: Response) {
  try {
    ...
    if (locationFilteringActive) {
      const listingsRaw = await TrainerListing.find(query).select("-adminNotes").lean();
      const requestedCity = typeof city === "string" ? city.trim().toLowerCase() : "";
      const requestedState = typeof req.query.state === "string" ? req.query.state.trim().toLowerCase() : "";
